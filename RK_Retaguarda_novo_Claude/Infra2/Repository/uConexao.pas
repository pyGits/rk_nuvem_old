...
uses
  ...
  FireDAC.Phys, FireDAC.Phys.FB, FireDAC.DApt,uIniManager;
...
begin
  ...
  IniDiretorio:='C:\System_RK\Server\retaguarda.ini';


  if not FileExists(IniDiretorio) then raise Exception.Create('Arquivo de Configuracao RETAGUARDA.ini Não encontrado !');

  Ini := TIniManager.getInstance(IniDiretorio);
  with FConexao do
  begin
  // Configuração da conexão com o Firebird
    DriverName := 'IB';
    Params.DriverID := 'IB'; // Define o driver Firebird
    Params.Database := Ini.ReadString('cfg','Banco','C:\System_RK\Server\BD\BANCO.FDB');
    Params.UserName := 'SYSDBA';
    Params.Password := 'masterkey';
    Params.Add('Server='+Ini.ReadString('cfg','IP','127.0.0.1'));
...
function TConexao.CreateQuery(const ASQL: string): TFDQuery;
begin
  Result := TFDQuery.Create(nil);
  Result.Connection := FConexao;
  Result.SQL.Text := ASQL;
...
function TConexao.ExecSQL(const ASQL: string): Integer;
var
  Q: TFDQuery;
begin
...
begin
  ...
  begin
    Params.DriverID := 'FB';
    Params.Database := 'C:\Dados\MEUBANCO.FDB';
    Params.UserName := 'SYSDBA';
    Params.Password := 'masterkey';
