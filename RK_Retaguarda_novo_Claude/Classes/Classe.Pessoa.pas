...
interface
uses Classe.Funcoes,System.SysUtils;
...
begin
  ...
  begin
    // se for CNPJ validar
    if ((ValidaCNPJ(Value)) or (Value = '11111111111111')) then
    begin
      CNPJCPF := Value;
    end
    else
    begin
      raise Exception.Create('Erro CNPJ não validado');
    end;
  end
  else
  if Value = '' then
...
//   else
//   begin
//      raise Exception.Create('Erro CPF não validado');
//   end;
  end;
