...
export class NotaFiscalItem {
  ...
  // Retorna as distribuições por loja; se vazio, considera toda a quantidade na loja informada (fallback).
  distribuicoesOuPadrao(lojaPadrao: number): ItemDistribuicao[] {
    if (this.distribuicoes && this.distribuicoes.length > 0) return this.distribuicoes;
    return [new ItemDistribuicao(Number(lojaPadrao), this.quantidadeEstoque())];
  }

  // Lojas distintas que recebem estoque deste item.
  lojasDistribuicao(lojaPadrao: number): number[] {
    return [...new Set(this.distribuicoesOuPadrao(lojaPadrao).map((d) => Number(d.lojaCodigo)))];
  }
  // Custo = valorProdutos
  //     + valorFrete
