...
export default class ContaReceberUseCase {
  ...
  async gerarRecibo(input: Input): Promise<Output> {
    ...
    // Nada de consulta de saldo aqui: tudo que o comprovante imprime sai do
    // proprio recibo, congelado no momento do recebimento. E o que faz a 2a via
    // repetir a 1a mesmo meses depois.
    const arquivo = await gerarReciboPDF(recibo, loja);
